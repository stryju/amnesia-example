...
(function ( factory ) {
  ...
  function amnesiaCacheProvider() {
    ...
    function $get( $cacheFactory, $timeout, $rootScope ) {
      // jshint validthis:true
      // jshint latedef:false
      ...
      function Amnesia( lifespan ) {
        if ( ! ( this instanceof Amnesia ) ) {
          return cached || ( cached = new Amnesia( lifespan ) );
        }

        var cache = $cacheFactory( 'αμνε$ια' + ( _i++ || '' ) );
        ...
        function ticktock( id ) {
          ...
          timeouts[ id ] = $timeout( function () {
            ...
            $rootScope.$emit( 'amnesia:remove', id );
          }, cache.amnesia.lifespan, false );

          $rootScope.$emit( 'amnesia:add', id );
        }
    ...
    $get.$inject = [
      '$cacheFactory', '$timeout', '$rootScope'
    ];
  }
